...
from utils.flow_viz import save_vis_flow_tofile, flow_to_image, save_flow_torch
import imageio
...
def inference_flow(model,
    ...
    for test_id in range(0, len(filenames) - 1):
        ...
        if inference_video is not None and save_video:
            ...
            flow2 = flow * 1.5
            flow_batch = torch.cat([flow, flow2], dim=0)  # [2, H, W, 2]
            save_flow_torch(flow_batch, "output/gmflow-scale2-regrefine6-davis/torch/")
            # save vis flow
