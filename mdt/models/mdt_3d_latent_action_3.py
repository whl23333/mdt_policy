...
class MDT3dLatentActionAgent(pl.LightningModule):
    ...
    def __init__(
        ...
        motion_transformer_ckpt: Optional[str] = '/data/250010208/whl/results/mdt_policy/logs/runs/2025-10-27/22-31-05/saved_models/epoch=42.ckpt',
        ...
        self.motion_transformer_ckpt = motion_transformer_ckpt
        ...
        if self.motion_transformer_ckpt is not None:
            component_states = {
                'motion_transformer': {},
                'embed_patch': {},
                'embed_lang': {},
            ...
            state_dict = torch.load(self.motion_transformer_ckpt, map_location='cpu')
            ckpt = state_dict['state_dict']
            for key, value in ckpt.items():
                if key.startswith('motion_transformer.'):
                    component_key = key.replace('motion_transformer.', '')
                    component_states['motion_transformer'][component_key] = value
                elif key.startswith('embed_patch.'):
                    component_key = key.replace('embed_patch.', '')
                    component_states['embed_patch'][component_key] = value
                elif key.startswith('embed_lang.'):
                    component_key = key.replace('embed_lang.', '')
                    component_states['embed_lang'][component_key] = value
                elif key.startswith('embed_mae.'):
                    component_key = key.replace('embed_mae.', '')
                    component_states['embed_mae'][component_key] = value
            missing_keys, unexpected_keys = self.motion_transformer.load_state_dict(component_states['motion_transformer'], strict=False)
            logger.info(f"Loaded motion_transformer from pretrained checkpoint with missing keys: {missing_keys}, unexpected keys: {unexpected_keys}")
            for params in self.motion_transformer.parameters():
                params.requires_grad = False
            self.motion_transformer.eval()
            m, e = self.embed_patch.load_state_dict(component_states['embed_patch'], strict=False)
            logger.info(f"Loaded embed_patch from pretrained checkpoint with missing keys: {m}, unexpected keys: {e}")
            for params in self.embed_patch.parameters():
                params.requires_grad = False
            self.embed_patch.eval()
            m, e = self.embed_lang.load_state_dict(component_states['embed_lang'], strict=False)
            logger.info(f"Loaded embed_lang from pretrained checkpoint with missing keys: {m}, unexpected keys: {e}")
            for params in self.embed_lang.parameters():
                params.requires_grad = False
            self.embed_lang.eval()
            m, e = self.embed_mae.load_state_dict(component_states['embed_mae'], strict=False)
            logger.info(f"Loaded embed_mae from pretrained checkpoint with missing keys: {m}, unexpected keys: {e}")
            for params in self.embed_mae.parameters():
                params.requires_grad = False
            self.embed_mae.eval()
    ...
    def configure_optimizers(self):
        ...
        if self.latent_motion_pred:
            optim_groups.extend([
                # {"params": self.motion_transformer.parameters(), "weight_decay": self.optimizer_config.transformer_weight_decay}, # motion_transformer is frozen
        ...
        # ])# commented out to freeze embedding layers
        ...
        for self.modality_scope, dataset_batch in batch.items():
            ...
            if self.latent_motion_pred:
                ...
                for t in range(latent_motion_emb.shape[1]):
                    ...
                    logger.info(f"Denormalizing with mean: {mean.flatten().cpu().numpy()}, std: {std.flatten().cpu().numpy()}")
