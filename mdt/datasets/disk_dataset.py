...
class ExtendedDiskDatasetLatent(DiskDataset):
    '''
    add extra future rgb image for latent action generation
    '''
    def __init__(
        self,
        *args: Any,
        ...
        **kwargs: Any,
        ...
        self.latent_action_diff = latent_action_diff
        # self.min_window_size = self.action_seq_len
        # self.max_window_size = self.action_seq_len + self.future_range
    ...
    def _load_episode(self, idx: int, window_size: int) -> Dict[str, np.ndarray]:
        ...
        # load latent future images from episodes
        latent_future_diff = self.latent_action_diff
        # latent_future_idx = [start_idx + self.obs_seq_len + (i+1)*latent_future_diff - 1 for i in range(self.latent_action_num)]
        # latent_future_episodes = [self.load_file(self._get_episode_name(file_idx)) for file_idx in latent_future_idx]
        # latent_future_static = [ep['rgb_static'] for ep in latent_future_episodes] # shape: [200, 200, 3]
        # latent_future_gripper = [ep['rgb_gripper'] for ep in latent_future_episodes] # shape: [84, 84, 3]
        # latent_future_static = np.stack(latent_future_static, axis=0)  # shape: [latent_action_num, 200, 200, 3]
        # latent_future_gripper = np.stack(latent_future_gripper, axis=0)  # shape: [latent_action_num, 84, 84, 3]
        latent_idx = [start_idx + self.obs_seq_len + i*latent_future_diff - 1 for i in range(self.latent_action_num + 1)] # len: latent_action_num + 1
        latent_episodes = [self.load_file(self._get_episode_name(file_idx)) for file_idx in latent_idx]
        latent_static = [ep['rgb_static'] for ep in latent_episodes]  # shape: [latent_action_num + 1, 200, 200, 3]
        latent_gripper = [ep['rgb_gripper'] for ep in latent_episodes]  # shape: [latent_action_num + 1, 84, 84, 3]
        latent_static = np.stack(latent_static, axis=0)  # shape: [latent_action_num + 1, 200, 200, 3]
        latent_gripper = np.stack(latent_gripper, axis=0)  # shape: [latent_action_num + 1, 84, 84, 3]
        ...
        episode['gen_gripper'] = gen_img_gripper
        episode['future_frame_diff'] = np.array(img_gen_frame_diff)
        episode['latent_static'] = latent_static
        episode['latent_gripper'] = latent_gripper
        episode['latent_action_num'] = np.array(self.latent_action_num)
        episode['latent_action_diff'] = np.array(self.latent_action_diff)
        return episode
